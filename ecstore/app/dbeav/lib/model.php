...
class dbeav_model extends base_db_model{
    ...
    function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null){
        ...
        $pkey = $this->_pkeys($this->table_name());
        if($pkey && $cols != $pkey){
            //组织新的查询字段写法,如t2.goods_id,t2.cat_id;如果有别名则不增加表的别名为前缀
            $arr_cols = explode(',',$cols);
            $newcols = '';
            foreach($arr_cols as $vcol){
                if( stripos(trim($vcol),' as ') || stripos(trim($vcol),' ') ){
                    $newcols .= ',' . $vcol;
                }else{
                    $newcols .= ',t2.' . $vcol;
                }
            }
            $newcols = ltrim($newcols,',');
            // 拼接limit,offset的sql语句
            if ($offset >= 0 || $limit >= 0){
            ...
            //如果有主键，先根据条件查出主键，再根据主键查询对应行的信息
            if($orderType)$ordersql = ' ORDER BY ' . (is_array($orderType) ? implode($orderType,' ') : $orderType);
            $selectIds_sql = 'SELECT '. $pkey . ' FROM `' . $this->table_name(true).'` WHERE ' . $this->_filter($filter);
            $sql = 'SELECT ' . $newcols . ' FROM (' . $selectIds_sql . $ordersql . $limit_sql . ') AS t1,' . $this->table_name(true) . ' AS t2 WHERE t1.' . $pkey . '=t2.' . $pkey;
...
//                    $obj->save($mconv,$subMustUpdate);
    ...
    private function _pkeys($tableName){
        $oDbTable = new base_application_dbtable;
        $itemdefine = $oDbTable->detect(($appId?$appId:$this->app->app_id),$tableName)->load(false);
        if(count($itemdefine['pkeys'])>1){
            return false;
        }
        if(is_array($itemdefine['pkeys'])){
            return reset($itemdefine['pkeys']);
        }else{
            return false;
        }
    }
