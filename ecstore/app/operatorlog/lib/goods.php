...
class operatorlog_goods
{
    ...
    function brand_log($newdata,$olddata){
        if(empty($newdata['brand_id'])){
            $this->objlog->logs('goods', '添加商品作者', '添加商品作者 '.$_POST['brand_name']);
        }else{
            $modify_flag = 0;
            $data = array();
            ...
            if($modify_flag>0){
                $memo  = "serialize".$this->delimiter."编辑商品作者ID {$_POST['brand_name']}".$this->delimiter.serialize($data);
                $this->objlog->logs('goods', '编辑商品作者', $memo);
            }
        }
    }
    ...
    function batchUpdateArray( $goods_id , $tableName, $updateName, $updateValue ){
        #↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓记录管理员操作日志,商品品牌@lujy↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        $goodsbn = implode(',',$goods_id);
        $memo = '商品ID为('.$goodsbn.')的(商品作者)全部修改为('.$updateValue['1'].')';

        $this->objlog->logs('goods', '批量编辑 商品作者', $memo);
        #↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑记录管理员操作日志,商品品牌@lujy↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
    }
