...
class b2c_ctl_admin_goods extends desktop_controller{
    ...
    function index(){
        ...
        if($this->has_permission('brandgoods')){
            $group[] = array('label'=>app::get('b2c')->_('商品作者'),'icon'=>'download.gif','submit'=>'index.php?app=b2c&ctl=admin_goods&act=singleBatchEdit&p[0]=brand','target'=>'dialog');
        }
    ...
    function singleBatchEdit($editType=''){
        ...
        switch( $editType ){
            ...
            case 'brand':
                ...
                if(!$this->has_permission('brandgoods')){
                    header('Content-Type:text/html; charset=utf-8');
                    echo app::get('b2c')->_("您无权批量操作商品作者");exit;
                }
