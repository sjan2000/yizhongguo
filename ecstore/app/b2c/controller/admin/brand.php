...
class b2c_ctl_admin_brand extends desktop_controller{
    ...
    function index(){
        $this->finder('b2c_mdl_brand',array(
            'title'=>app::get('b2c')->_('商品作者'),
            'actions'=>array(
                array('label'=>app::get('b2c')->_('添加作者'),'icon'=>'add.gif','href'=>'index.php?app=b2c&ctl=admin_brand&act=create','target'=>'_blank'),
    ...
    function save(){
        ...
        if(empty($_POST['brand_id']) && is_array($brandname)){
             $this->end(false,app::get('b2c')->_('作者名重复'));
        }
        ...
        if($objBrand->save($data)){
            ...
            #↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑记录管理员操作日志@lujy↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
            $this->end(true,app::get('b2c')->_('作者保存成功'));
        }else{
            $this->end(false,app::get('b2c')->_('作者保存失败'));
        }
    }

    function edit($brand_id){
        $this->path[] = array('text'=>app::get('b2c')->_('商品作者编辑'));
        $objBrand = $this->app->model('brand');
