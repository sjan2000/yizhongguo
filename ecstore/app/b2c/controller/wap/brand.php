...
class b2c_ctl_wap_brand extends wap_frontpage{
    ...
    function __construct($app){
        ...
        if(isset($shopname)){
            $this->title = app::get('b2c')->_('作者页').'_'.$shopname;
            $this->keywords = app::get('b2c')->_('作者页').'_'.$shopname;
            $this->description = app::get('b2c')->_('作者页').'_'.$shopname;
        }
    ...
    public function showList($page=1){
        ...
        $this->path[] = array('title'=>app::get('b2c')->_('作者专区'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'wap_brand', 'act'=>'showlist','full'=>1)));
        $GLOBALS['runtime']['path'] = $this->path;

        $title=$title['title']?$title['title']:app::get('b2c')->_('作者');
        $this->pagedata['pager'] = array(
            'current'=>$page,
            'total'=>ceil($brandCount/$pageLimit),
            'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'wap_brand', 'act'=>'showList','full'=>1,'args'=>array(($tmp = time())))),
            'token'=>$tmp
            );
        ...
        $this->setSeo('wap_brand','showList',$this->prepareListSeoData($this->pagedata));
        $this->page('wap/brand/showList.html');
    }
    ...
    public function index($brand_id, $page=1,$orderBy=1,$view='') {
        ...
        if(!empty($seo_info['seo_info']['seo_title']) || !empty($seo_info['seo_info']['seo_keywords']) || !empty($seo_info['seo_info']['seo_description'])){
            ...
            $this->setSeo('wap_brand','index',$this->prepareSeoData($this->pagedata));
        }
        ...
        $this->page('wap/brand/index.html');
